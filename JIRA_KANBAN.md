...
* **[E3S1T8] Compile and validate generated PLC logic with OpenPLC Editor / Runtime (primary target)** — complete.
  * Added `src/openplc_checker.py`: an OpenPLC v3 integration wrapper with `is_openplc_available()` (discovers a local OpenPLC install with `webserver/core/openplc` + `scripts/compile_program.sh`) and `run_openplc_simulation(st_path, run_seconds=1.0) -> SimulationResult`. It strips Matiec-incompatible `//` comments, wraps standalone `PROGRAM` blocks in a minimal `CONFIGURATION`/`RESOURCE`/`TASK`, compiles via the OpenPLC `compile_program.sh`, runs `core/openplc` under `timeout`, and returns whether it ran scan cycles without a crash; compiled outputs and `active_program` are backed up and restored.
  * Added `tests/test_openplc_simulation.py` (3 tests, `skipUnless(is_openplc_available())`): deterministic + `signal_light_demo` hybrid ST compile and simulate stably under OpenPLC; `sample_control` hybrid is marked `expectedFailure` — it carries the same `REAL >= INT` literal (`tank_level_sensor >= 80`) the OpenPLC `iec2c` rejects (mirrors E3S1T7). Full suite now 92 tests.
* **[E3S1T9] TIA Portal / PLCSIM validation — documentation-only feasibility study (lower priority)** — skipped.
  * Marked **skipped** for submission: Siemens TIA Portal / PLCSIM is Windows-only, licensed commercial software that cannot run inside the Linux/WSL environment. The scope is addressed as a theoretical feasibility study — describing how the generated artifacts (ST, LD IR, PLCopen XML) would map into TIA Portal and how PLCSIM could provide closed-loop simulation — without attempting a live run. E3S1 is complete apart from this skipped item.

#### E3S2: PLCopen XML export
* **[E3S2T1] Create export module mapping PLC_AST to interoperable PLCopen XML** — complete.
  * Added `src/plcopen_xml_exporter.py`: `export_ast_to_plcopen_xml(ast_path, output_path=None) -> Path` serializes a `PLC_AST` into a PLCopen TC6 XML 2.01 document (namespace `http://www.plcopen.org/xml/tc6_0201`, with `xsi:schemaLocation`) using only the standard library. Populates `fileHeader`/`contentHeader`, POU interface `localVars` from AST devices (sanitized names, `BOOL` types), and the POU body `<ST>` with the deterministic Structured Text logic (including traceability comments) generated by `src.st_gen`. `export_all_asts()` batch-converts `data/ast/*.json` -> `data/plc/xml/*.xml`.
  * Added `tests/test_plcopen_xml_export.py` (7 tests): well-formed XML + root/schemaLocation, required sections (`project`/`fileHeader`/`contentHeader`/`types > pous > pou`/`interface`/`body > ST`), `localVars` matching AST devices, and ST-body traceability referencing each AST sequence step. Full suite now 99 tests.
* **[E3S2T2] Validate generated PLCopen XML** — complete.
  * Added `src/plcopen_xml_validator.py`: `validate_plcopen_xml(xml_path) -> ValidationResult` (lxml with stdlib `xml.etree` fallback) checks each exported PLCopen set against the TC6 v2.01 required-element specification — namespace/`schemaLocation`, `fileHeader`/`contentHeader` metadata completeness, program POU with interface `localVars` (named, typed variables), and non-empty `body > ST` containing `PROGRAM`/`END_PROGRAM` and traceability comments; `validate_plcopen_directory()` validates every `data/plc/xml/*.xml`. All 10 exported XML files validate cleanly.
  * Added `tests/test_plcopen_xml_validation.py` (9 tests): all exported XML validate; namespace/project sections; header metadata + contentHeader/pou name consistency; POU variable names/types and matching AST devices; ST body integrity and AST provenance traceability per sequence step. Full suite now 108 tests.
...
* **[E3S3T1] Build a two-tier validation framework** — complete.
  * **Tier 1 — Deterministic grounding & schema validation (complete):** added `src/validation_framework.py` — `Tier1Validator.validate_artifacts(parsed, ast, st_text, ld) -> Tier1Result` consolidates equipment grounding (parsed `equipment_list` -> AST `source_equipment` verbatim, ST/LD variable resolution, unknown device-tag rejection), ID continuity (continuous step_id, interlock `priority >= 1`, unique LD network_id), authoritative field protection (verbatim interlock `source_interlock_condition` / sequence `source_step_id` into the parsed requirement), and the structural contract (`PROGRAM`/`END_PROGRAM`, balanced IF/END_IF, sequence-before-safety). Pydantic report models: `Tier1Result` / `ValidationIssue` / `Severity`. `tests/test_validation_framework.py` (13 tests): both real pipelines pass; ungrounded devices, broken sequences, tampered interlocks, duplicate LD networks, and unbalanced ST are rejected with structured codes. Full suite now 121 tests.
  * **Tier 2 — LLM-based semantic intent evaluator (complete):** added `Tier2SemanticEvaluator` to `src/validation_framework.py` using LangChain `with_structured_output` (E2S3 Approach C pattern): the LLM inspects generated ST/LD logic against the original requirement and returns structured verdicts (`Tier2Evaluation` -> normalized `Tier2Result`: per-step coverage, per-interlock safety checks, missing-step alerts, critical-hazard flags). Added `run_unified_validation(parsed, ast, st_text, ld, backend, score_threshold=80.0) -> UnifiedReport`: fails fast on Tier 1 structural errors (Tier 2 skipped), then applies the Python-owned gate — fails when the coverage score is below threshold or any critical hazard is flagged. Tests inject a deterministic fake evaluator so gating is verified offline; no live API calls in the suite. Full suite now 131 tests.
* **[E3S3T2] Add unit tests for parsers and generators** — complete.
  * Added 9 unit-test files covering the deterministic and mockable surface of every E2S1-E2S4 parser/generator: `tests/test_req_parser_units.py`, `test_gherkin_gen_units.py`, `test_ast_gen_A_units.py`, `test_ast_gen_B_units.py`, `test_ast_gen_C_units.py` (+ `ast_builders`), `test_st_gen_units.py`, `test_ld_ir_gen_units.py`, `test_st_gen_llm_direct_units.py`, and `test_ld_ir_gen_llm_direct_units.py` (236 new tests).
  * Deterministic helpers are tested directly: `req_parser` path/arg/backend/`get_wsl_host_ip` logic; `gherkin_gen` content-word grounding, fabricated-given backstop, pure `.feature` renderer; `ast_gen_B` tokenization, Dice matching, boundary-safe name matching, condition extraction, and pipeline `build_ast`; `ast_gen_C` tool-call extraction/parsing (dict/string args, OpenAI kwargs shape, failure counts), grounding checks, builders, and scripted-fake-LLM `build_ast`; `st_gen` variable sanitization/mapping, action classification, block construction, and program render; `ld_ir_gen` term/negation matching, unsupported-condition detection, placeholder coils, multi-target interlock split networks, and program ordering.
  * LLM boundaries are exercised offline with LangChain callables/RunnableLambda coercion and scripted fake backends: per-item scenario provenance stamping and grounding (gherkin), `generate_st` cleanup/validation/guards, `validate_ld_structure` ordering/uniqueness/traceability/multi-target completeness, and the LD validation-retry loop. Fatal paths (`sys.exit`/exception contracts) are covered with `tmp` files; no live API calls in the suite. Full suite now 363 tests.
* **[E3S3T3] Add negative / failure-path test cases** — deferred / skipped.
  * Marked **deferred / skipped** for submission: the core deterministic negative gates are already covered by the Tier 1 framework (`tests/test_validation_framework.py` — ungrounded devices, broken sequences, tampered interlocks, duplicate LD networks, unbalanced ST) and by the E3S3T2 generator unit suites (fatal input paths, invalid tool arguments, grounding rejections, and structural-validation failures).
  * Broader edge-case fuzzing with more complex industrial requirement texts (ambiguous/incomplete requirements, contradictory interlock conditions, missing equipment information) remains future work.
